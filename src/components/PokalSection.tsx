...
export default function PokalSection({ pokale, ownerName }: Props) {
  if (!pokale.length) return null;

  const sorted = [...pokale].sort(
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {sorted.map((p) => {
          const boxSize = p.isSeries ? 132 : 78;
          const imgSize = p.isSeries ? 108 : 64;
          return (
            <div
              key={p.id}
              className={`flex flex-col items-center gap-2 rounded-2xl border p-3 ${
                p.isSeries
                  ? "border-amber-500/30 bg-amber-500/[0.06] col-span-2 sm:col-span-1"
            ...
            >
              ...
              >
                {p.isSeries ? "Eventreihe" : "Event"} · {getPokalCategoryLabel(p.category)}
              </span>
              <div className="text-center leading-tight">
                <p
                  className="font-serif text-xs text-gray-200"
                  style={{ fontVariant: "small-caps" }}
                >
                  {p.title}
                </p>
                <p
                  className="font-serif text-[10px] text-gray-500 mt-0.5"
                  style={{ fontVariant: "small-caps" }}
                >
                  {ownerName}
                </p>
