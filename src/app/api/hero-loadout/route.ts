...
// Eigenes Loadout anlegen/ändern -- Archetyp ist jederzeit wechselbar,
// ebenso die Ausrüstung je Slot (leerer Wert für einen Slot entfernt ihn
// wieder).
export async function POST(req: NextRequest) {
  ...
  if (!body.archetypeId?.trim()) {
    return NextResponse.json({ error: "archetypeId ist Pflicht" }, { status: 400 });
  }

  const archetype = await prisma.heroArchetype.findUnique({ where: { id: body.archetypeId.trim() } });
  if (!archetype) return NextResponse.json({ error: "Archetyp nicht gefunden" }, { status: 400 });

  const loadout = await prisma.$transaction(async tx => {
    const upserted = await tx.userHeroLoadout.upsert({
      where: { userId: user.id },
      create: { id: randomUUID(), userId: user.id, archetypeId: body.archetypeId!.trim() },
      update: { archetypeId: body.archetypeId!.trim() },
    });
