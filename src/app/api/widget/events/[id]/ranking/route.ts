...
/**
 * GET /api/widget/events/[id]/ranking
 *
 * Live-Rangliste eines einzelnen Events fürs Widget — Formel 1:1 aus
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const cfg: Record<string, number | { coins: number; points: number }> = event.pointsConfig
    ? JSON.parse(event.pointsConfig)
    : {};
  ...
  if (format === "ffa" || format === "coop_stats" || format === "avg_stats") {
    ...
    for (const match of event.matches) {
      for (const uid of [match.player1Id, match.player2Id, match.winnerId]) {
        if (uid && !userMap.has(uid)) userMap.set(uid, { id: uid, name: null, username: null, image: null, rankPoints: 0 });
      }
    }
    ...
    for (const match of event.matches) {
      ...
      if (!userMap.has(p1)) userMap.set(p1, { id: p1, name: null, username: null, image: null, rankPoints: 0 });
      if (!userMap.has(p2)) userMap.set(p2, { id: p2, name: null, username: null, image: null, rankPoints: 0 });
      if (match.winnerId === p1) {
        pts.set(p1, pts.get(p1)! + 3);
      } else if (match.winnerId === p2) {
        pts.set(p2, pts.get(p2)! + 3);
      } else if (match.score1 !== null && match.score2 !== null && match.score1 === match.score2) {
        pts.set(p1, pts.get(p1)! + 1);
        pts.set(p2, pts.get(p2)! + 1);
      }
    }
    ranking = [...userMap.keys()]
