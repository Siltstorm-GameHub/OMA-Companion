...
export default async function OverlaySettingsPage({
  ...
  // (sehr alter Link) bleiben Flip-Kachel-Rückseite und Lieblingsspiele einfach aus der
  // Auswahl, statt die Seite zum Absturz zu bringen.
  const streamer = streamerId
    ? await prisma.user.findUnique({
        where: { id: streamerId },
        select: { id: true, name: true, username: true, favoriteGamesJson: true },
      })
    : null;
