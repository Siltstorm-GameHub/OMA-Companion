...
import { Check, ShoppingCart, Loader2, Star, Lock, Send, Target, Heart, Clock, AlertTriangle, Gamepad2, Pencil } from "lucide-react";
      ...
      <div className="relative p-4">
        ...
        {daysLeft !== null && daysLeft <= 14 && daysLeft > 0 && (
          <div className="absolute top-0 inset-x-0 bg-gradient-to-r from-orange-500/20 to-transparent px-3 py-1 text-[10px] text-orange-300 font-medium flex items-center gap-1">
            <Clock className="w-3 h-3" /> Noch {daysLeft} {daysLeft === 1 ? "Tag" : "Tage"}
          </div>
        )}
        ...
        <div className="flex items-center justify-between gap-2 mt-auto pt-1">
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-500/8 border border-amber-500/15">
            ...
            <span className={`text-sm font-bold tabular-nums ${canAfford || alreadyOwned ? "text-amber-300" : "text-red-400"}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            {alreadyOwned && item.type === "status_message" && (
              <button
                ...
                className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-medium glass border border-blue-500/25 text-blue-300 hover:border-blue-400/40 transition-all"
              >
            ...
            {!alreadyOwned && (
              <button
                ...
                className={`flex items-center gap-1.5 text-xs px-4 py-2 rounded-xl font-semibold transition-all active:scale-[0.97] disabled:opacity-50 ${
                  notYetAvailable
                    ...
                    : "bg-rose-600 hover:bg-rose-500 text-white shadow-[0_0_16px_rgba(244,63,94,0.3)] hover:shadow-[0_0_20px_rgba(244,63,94,0.45)]"
                }`}
              >
                {loading          ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> :
                 notYetAvailable  ? <><Lock className="w-3.5 h-3.5" /> Bald</> :
                 soldOut          ? <><Lock className="w-3.5 h-3.5" /> Ausverkauft</> :
                 !canAfford       ? <><Lock className="w-3.5 h-3.5" /> Zu teuer</> :
                 <><ShoppingCart className="w-3.5 h-3.5" /> Kaufen</>}
              </button>
            )}
        ...
        {item.stock !== null && item.stock <= 5 && item.stock > 0 && (
          <p className="text-[10px] text-amber-500 mt-2 flex items-center gap-1">
            <AlertTriangle className="w-3 h-3" /> Nur noch {item.stock} verfügbar
          </p>
        )}
        {soldOut && (
