...
import { Clapperboard, Check, ExternalLink } from "lucide-react";
...
export default function ClipVotingClient({ contestId, nominations, initialVoteId, isLoggedIn }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {nominations.map((nom) => {
        ...
        const creatorName = nom.submittedBy?.name ?? nom.submittedBy?.username ?? nom.twitchCreatorLogin ?? "Unbekannt";
          ...
          >
            ...
            <div className="relative aspect-video bg-white/[0.03]">
              {nom.thumbnailUrl ? (
                ...
                <img
                  src={nom.thumbnailUrl.replace("%{width}", "480").replace("%{height}", "270")}
                  alt={nom.clipTitle ?? "Clip"}
            ...
            <div className="p-3 space-y-2">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="text-xs text-gray-500 flex items-center gap-1.5 min-w-0">
                  ...
                  {nom.partnerTwitchLogin && (
                    <span className="text-[#9146ff] shrink-0">· {nom.partnerTwitchLogin}</span>
                  )}
                </div>
