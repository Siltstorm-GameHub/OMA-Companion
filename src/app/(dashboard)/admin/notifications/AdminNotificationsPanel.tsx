...
import { Send, Users, X, ChevronDown, Check } from "lucide-react";
...
export default function AdminNotificationsPanel() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (linkDropRef.current && !linkDropRef.current.contains(e.target as Node)) {
  ...
  async function send() {
    ...
    if (mode === "user" && selectedUsers.length === 0) return;
    setLoading(true);
    setResult(null);
    try {
  ...
  const canSend = title.trim() && body.trim() && (mode === "all" || selectedUsers.length > 0);
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {selectedUsers.map(u => (
                <div key={u.id}
                  ...
                  {u.image && (
                    <Image src={u.image} alt="" width={16} height={16} className="rounded-full" />
                  )}
          ...
          <div ref={searchRef} className="relative">
            ...
            {showSuggestions && (query || suggestions.length > 0) && (
              <div className="absolute z-50 left-0 right-0 mt-1 rounded-xl overflow-hidden shadow-xl"
                ...
                {loadingSearch && (
                  <div className="px-3 py-2 text-xs text-gray-500">Suche…</div>
                )}
