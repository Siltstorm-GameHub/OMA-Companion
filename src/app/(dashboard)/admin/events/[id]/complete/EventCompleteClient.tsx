...
import {
  ...
  ListOrdered, GripVertical, Coins, AlertTriangle, RotateCcw,
} from "lucide-react";
...
export default function EventCompleteClient({
  ...
  isReEdit, initialData, initialFinalRanking, initialFinalRankingNote,
}: Props) {
  ...
  const [pollVotes, setPollVotes] = useState<Record<string, number>>(() => {
    // Pre-fill 1 vote for old winners on re-edit
    ...
    try {
      const res = await fetch(`/api/admin/events/${eventId}/complete`, {
        ...
        body: JSON.stringify({
          ...
          pollWinnerIds:           pollConfig.enabled && pollWinners.length > 0 ? pollWinners : undefined,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>/</span>
        <span className="text-gray-300">{isReEdit ? "Abschluss bearbeiten" : "Event abschließen"}</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
        ...
        <div className="space-y-4">
          ...
          {tournamentStatFields.length > 0 && (
            <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(245,158,11,0.05)", border: "1px solid rgba(245,158,11,0.15)" }}>
              ...
              <div>
                ...
                <select value={winnerStatField} onChange={e => { setWinnerStatField(e.target.value); if (!rankingManuallyEdited && e.target.value) { setRankingOrder(prev => [...prev].sort((a, b) => (userStats[b]?.[e.target.value] ?? 0) - (userStats[a]?.[e.target.value] ?? 0))); } }} className={inputCls}>
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-2">
            <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-widest">
              {isReEdit ? "Wird aktualisiert" : "Wird vergeben"}
            </p>
            {isReEdit ? (
              ...
              <ul className="space-y-1 text-xs text-gray-400">
                ...
                {rewardsConfig.placements.map(p => (
                  <li key={p.place} className="flex items-center gap-2">
                    <span className="shrink-0">{MEDALS[p.place - 1] ?? `${p.place}.`}</span>
                    {p.place}. Platz:{" "}
                    {p.coins > 0 ? `${p.coins} Münzen` : ""}
                    {p.coins > 0 && p.rankPoints > 0 ? " + " : ""}
                    {p.rankPoints > 0 ? `${p.rankPoints} Punkte` : ""}
                    {p.coins === 0 && p.rankPoints === 0 ? "keine Belohnung" : ""}
                  </li>
                ))}
                {pollConfig.enabled && (
                  <li className="flex items-center gap-2">
        ...
        <div className="space-y-3">
          <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(59,130,246,0.05)", border: "1px solid rgba(59,130,246,0.15)" }}>
            ...
            <p className="text-[10px] text-gray-600">Drag & Drop oder Pfeile nutzen</p>
            ...
            {registeredUsers.length === 0 ? (
              ...
              <div className="space-y-1">
                {rankingOrder.map((uid, idx) => {
                  ...
                  if (!u) return null;
                  const reward = rewardsConfig.placements.find(p => p.place === idx + 1);
                  return (
                    <div
                      key={uid}
                      draggable
                      ...
                      className="flex items-center gap-2 px-2 py-2 rounded-lg bg-white/[0.03] border border-white/[0.05] cursor-grab active:cursor-grabbing select-none"
                    >
