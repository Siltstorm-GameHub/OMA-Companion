    ...
    <div className="space-y-4">
      ...
      {isPollOnly && (
        <div className="flex items-start gap-2 rounded-xl border border-violet-500/20 bg-violet-500/5 px-4 py-3 text-xs text-violet-300">
          ...
          <span>
            Die <strong>Spielphase ist bereits abgeschlossen</strong> – Teilnahmen, Platzierungen und die Gesamttabelle wurden bereits aktualisiert.
            Hier können die Umfrageergebnisse nachgetragen werden.
          </span>
        </div>
      )}
      {isReEdit && !isPollOnly && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
        ...
        <div className="space-y-4">
          ...
          {/* Gewinner-Stat – hidden in poll-only mode */}
          {!isPollOnly && tournamentStatFields.length > 0 && (
          ...
          {/* MVP – hidden in poll-only mode */}
          {!isPollOnly && seriesStatConfig?.mvpStatField && (
          ...
          {isPollOnly ? (
            <button
              ...
              disabled={loading || pollWinners.length === 0}
              className="w-full flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold text-white bg-violet-700 hover:bg-violet-600 transition-colors disabled:opacity-50"
            >
              <Vote className="w-4 h-4" />
              {loading ? "Wird gespeichert…" : "Umfrage abschließen"}
            </button>
          ) : (
        ...
        <div className="space-y-3">
          <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(59,130,246,0.05)", border: "1px solid rgba(59,130,246,0.15)" }}>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isPollOnly && <span className="text-[10px] text-gray-600 ml-1">(schreibgeschützt)</span>}
              {!rankingManuallyEdited && winnerStatField && !isPollOnly && (
                <span className="text-[10px] text-gray-600">— auto-sortiert nach „{winnerStatField}"</span>
              )}
              {rankingManuallyEdited && !isPollOnly && (
            ...
            {!isPollOnly && <p className="text-[10px] text-gray-600">Drag & Drop oder Pfeile nutzen · <Equal className="w-3 h-3 inline" /> = Gleichstand</p>}
                ...
                {rankingOrder.map((uid, idx) => {
                  ...
                  return (
                    <div key={uid}>
                      ...
                      <div
                        draggable={!isPollOnly}
                        onDragStart={() => !isPollOnly && setDragIdx(idx)}
                        onDragOver={e => e.preventDefault()}
                        onDrop={() => {
                          if (isPollOnly || dragIdx === null || dragIdx === idx) return;
                          setRankingManuallyEdited(true);
                          setRankingOrder(prev => {
                            const next = [...prev];
                        ...
                        } ${isPollOnly ? "cursor-default opacity-70" : "cursor-grab active:cursor-grabbing"}`}
                      >
                        {!isPollOnly && <GripVertical className="w-3.5 h-3.5 text-gray-600 shrink-0" />}
