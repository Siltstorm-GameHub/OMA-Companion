...
export default function ServerCredentials({
  ...
  return (
    <div className="space-y-3">
      {/* Primär: manuelle Verbindungsdaten — funktioniert immer, unabhängig von Steam/Protokoll-Handlern */}
      ...
      {/* Sekundär: optionaler Ein-Klick-Connect, hängt von der steam://-Verknüpfung des Systems ab */}
      {connectLink && (
        <div className="space-y-1.5 pt-1 border-t border-white/[0.05]">
          <p className="text-[10px] text-gray-600 uppercase tracking-wide">Optional: automatisch verbinden</p>
          <a
            href={connectLink}
            onClick={markConnected}
            className="flex items-center justify-center gap-2 w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/8 text-gray-300 text-sm font-medium transition-colors"
          >
            <ExternalLink className="w-3.5 h-3.5" />
            Über Steam verbinden
          </a>
          <p className="text-[11px] text-gray-500">
            Funktioniert nur, wenn <code className="text-gray-400">steam://</code>-Links auf deinem PC mit Steam verknüpft sind. Falls nicht: oben Host/Passwort manuell im Spiel eintragen.
          </p>
        </div>
      )}
