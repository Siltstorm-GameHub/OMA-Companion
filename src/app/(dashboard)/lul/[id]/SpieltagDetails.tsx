...
export default function SpieltagDetails({
  ...
  // Determine max rounds for round-score display
    ...
    <div className="mt-2 border-t border-white/5 pt-2">
      ...
      {open && (
        <div className="mt-2 overflow-x-auto -mx-1 px-1">
          <table className="w-full text-[11px]" style={{ minWidth: isStatFmt ? `${200 + (isAvg ? 70 : statFields.length * 60)}px` : `${200 + maxRounds * 40}px` }}>
            <thead>
              <tr className="text-gray-600 border-b border-white/5">
                ...
                {!isStatFmt && maxRounds > 1 && (
                  <th className="text-center py-1 px-2 font-medium">Σ</th>
                )}
              </tr>
            </thead>
            <tbody>
              {players.map((entry, i) => {
                ...
                let statCells: React.ReactNode = null;
                if (isStatFmt && entry.statsJson) {
                  const s = (() => { try { return JSON.parse(entry.statsJson) as Record<string, unknown>; } catch { return {}; } })();
                  const nR = typeof s._rounds === "number" && s._rounds > 0 ? s._rounds : 1;

                  if (isAvg) {
                    const combined = calcCombinedAvg(entry.statsJson, statFields);
                    statCells = (
                      <td className="text-center py-1 px-2 tabular-nums font-bold" style={{
                        color: i === 0 ? "#fbbf24" : i === 1 ? "#d1d5db" : i === 2 ? "#b45309" : "#6b7280"
                      }}>
                        {combined >= 0 ? (combined % 1 === 0 ? combined : combined.toFixed(2)) : "–"}
                      </td>
                    );
                  } else {
                    statCells = statFields.map(f => {
                      const val = s[f];
                      const total = Array.isArray(val)
                        ? val.reduce((sum: number, v) => sum + (Number(v) || 0), 0)
                  ...
                  <tr key={entry.id} className="border-b border-white/[0.03] last:border-0">
                    ...
                    <td className="py-1 pr-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        {entry.gameWinner && <span className="text-amber-400 text-[10px]">🏆</span>}
                      </div>
                    </td>
                    {isStatFmt
                      ? statCells
